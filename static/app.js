...
const investigationFields = [['plan_number','Plan number'],['parcel','Parcel'],['owner','Owner'],['named_gc','Verified GC (manual)'],['related_permits','Later permits / trades'],['next_contact','Suggested first contact']];
...
function render() {
  ...
  const filtered = permits.filter(p => (!query || [p.description,p.address,p.city,p.county,p.permit_id,p.permit_type].some(v => v.toLowerCase().includes(query))) && (!$('status').value || p.status === $('status').value) && (!$('county').value || p.county === $('county').value) && (!$('stage').value || ($('stage').value === 'source_to_be_bid' ? p.source_contractor?.trim().toUpperCase() === 'TO BE BID' : p.opportunity_stage === $('stage').value)));
  const pages = Math.max(1, Math.ceil(filtered.length / PAGE_SIZE));
  page = Math.min(page, pages);
  const visible = filtered.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE);
  $('results').innerHTML = filtered.length ? visible.map(p => `<article class="record"><div><h2>${escapeHtml(p.description)}</h2><div class="meta"><b>${escapeHtml([p.city,p.county].filter(Boolean).join(' · ') || 'Arizona')}</b> · ${escapeHtml(p.address || 'Address unavailable')}<br>${escapeHtml(p.source)} · ${escapeHtml(p.permit_id)} · ${escapeHtml(p.issued_date || 'Date unavailable')} · ${escapeHtml(p.permit_type || 'Type unspecified')}${p.value ? ` · ${escapeHtml(p.value)}` : ''}</div></div><span class="badge">${p.source_contractor?.toUpperCase() === 'TO BE BID' ? 'Source: TO BE BID · ' : ''}${escapeHtml(p.status)}${p.opportunity_stage && p.opportunity_stage !== 'unverified' ? ` · ${escapeHtml(stages[p.opportunity_stage] || p.opportunity_stage)}` : ''}</span><div class="record-actions">${p.source_url ? `<a href="${escapeHtml(p.source_url)}" target="_blank" rel="noopener noreferrer">View source ↗</a>` : '<span class="meta">No source URL provided</span>'}<button type="button" data-edit="${p.id}">Review / notes</button></div><div class="detail" id="detail-${p.id}" hidden><label>Status<select id="status-${p.id}">${['new','reviewing','qualified','dismissed'].map(s => `<option value="${s}" ${p.status === s ? 'selected' : ''}>${s[0].toUpperCase()+s.slice(1)}</option>`).join('')}</select></label><p class="review-hint">Source contractor: <strong>${escapeHtml(p.source_contractor || 'Not provided')}</strong>. This is the source label, not a verified general contractor.</p><label>Opportunity stage<select id="stage-${p.id}">${Object.entries(stages).map(([value,label]) => `<option value="${value}" ${p.opportunity_stage === value ? 'selected' : ''}>${escapeHtml(label)}</option>`).join('')}</select></label>${investigationFields.map(([key,label]) => `<label>${label}<input id="${key}-${p.id}" maxlength="500" value="${escapeHtml(p[key])}"></label>`).join('')}<p class="review-hint">TO BE BID is a manual finding, not inferred from missing GC data. Check the Phoenix plan and later permits first; record the link or evidence in notes.</p><label>Notes / evidence<textarea id="notes-${p.id}" maxlength="2000">${escapeHtml(p.notes)}</textarea></label><button type="button" data-save="${p.id}">Save review</button></div></article>`).join('') : `<div class="empty"><strong>${permits.length ? 'No permits match these filters' : 'No permits imported yet'}</strong>${permits.length ? 'Try a different search, status, or opportunity filter. Source TO BE BID and verified TO BE BID are different.' : 'Import a CSV from a public permit source to start reviewing projects.'}</div>`;
